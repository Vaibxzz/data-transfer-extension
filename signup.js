...
const auth = window.__KOSH__?.auth || (typeof firebase !== 'undefined' && firebase.auth && firebase.auth());
const db = window.__KOSH__?.firestore || (typeof firebase !== 'undefined' && firebase.firestore && firebase.firestore());
...
// ====== GOOGLE AUTH (client-side using Firebase) ======
async function initiateGoogleAuth() {
  const btn = document.querySelector(".google-btn");
  if (btn) { btn.textContent = "Authenticating..."; btn.disabled = true; }

  try {
    if (!auth || !firebase || !firebase.auth) throw new Error("Firebase auth not initialized");
    const provider = new firebase.auth.GoogleAuthProvider();
    const result = await auth.signInWithPopup(provider);
    const user = result.user;
    const idToken = await user.getIdToken();
    await sendFirebaseIdTokenToBackend(idToken);
    authManager.setAuthenticated(
...
async function handleOutlookAuth() {
  ...
  const clientId = "663c904c-c20d-4a9b-9529-67f0d144462d"; // replace with your Azure app clientId if different
  ...
  const timer = setInterval(() => {
    try {
      ...
      // cross-origin until popup lands on our origin
    }
  }, 500);
}
