...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ---------- DATE PICKER (Flatpickr) ----------
        ...
        (function setupFlatpickr() {
            ...
            if (!datePickerDiv || !dateRange || !dateInput) {
              console.warn('Date picker elements missing; skipping Flatpickr init.');
              return;
            }
      
            // helper to parse simple date strings used in your UI
            function normalizeTimestamp(value) {
              ...
              if (m) {
                ...
                const dd = new Date(year < 100 ? year + 2000 : year, month, day);
            ...
            if (typeof flatpickr === 'undefined') {
              // flatpickr not loaded — attach safe click handler
              datePickerDiv.addEventListener('click', (e) => {
                e.preventDefault();
                alert('Calendar library not loaded. Please include flatpickr.min.js.');
              });
              console.warn('flatpickr not found on page.');
              return;
            }
      
            // parse any existing date-range text (optional)
            ...
            const fp = flatpickr(dateInput, {
              ...
              onChange: function(selectedDates, dateStr, instance) {
                ...
                // call your filtering hook if present
                if (typeof applyFiltersAndSort === 'function') applyFiltersAndSort();
              }
            });
      
            // clicking the visual date-picker opens flatpickr
    ...
    // NAV TABS behavior — redirect to options.html when Configuration is clicked
document.querySelectorAll('.nav-tab').forEach(tab => {
    tab.addEventListener('click', (e) => {
      ...
      if (label === 'Configuration') {
        // If running inside an extension (chrome runtime), open extension options
        if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.getURL) {
          try {
            // open options page inside extension
            chrome.tabs.create({ url: chrome.runtime.getURL('options.html'), active: true });
          } catch (err) {
            // fallback to runtime.openOptionsPage if available
            if (chrome.runtime.openOptionsPage) chrome.runtime.openOptionsPage();
            else window.location.href = 'options.html';
          }
        } else {
          // Web fallback: try relative path first (works when options.html is alongside dashboard.html)
          // then try site-root absolute as a fallback
          const rel = 'options.html';
          const absRoot = window.location.origin + '/options.html';
  
          // attempt relative -> absolute
          fetch(rel, { method: 'HEAD' }).then(resp => {
            if (resp.ok) {
              window.location.href = rel;
            } else {
              // try absolute root
              fetch(absRoot, { method: 'HEAD' }).then(aresp => {
                if (aresp.ok) window.location.href = absRoot;
                else {
                  // final fallback: just navigate relative (likely 404) but this is last resort
                  window.location.href = rel;
                }
              }).catch(() => { window.location.href = rel; });
            }
          }).catch(() => {
            // network fetch failed (CORS/local), just navigate relative
            window.location.href = rel;
          });
        }
  
        // small UI reset so tab doesn't stay selected
        setTimeout(() => {
          document.querySelectorAll('.nav-tab').forEach(t => t.classList.remove('active'));
          // keep the first tab active as default
