...
(async () => {
  try {
    ...
    console.warn('Firebase SDK not ready (continuing, will use stored auth if present).', e);
  }
})();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderFilteredData(){
    ...
    if (!filteredEntries || filteredEntries.length === 0) {
      ...
      filteredEntries.forEach(item => {
        const row = document.createElement('tr');
        row.innerHTML = `<td>${escapeHtml(item.firstName||'')}</td><td>${escapeHtml(item.lastName||'')}</td><td>${escapeHtml(item.country||'')}</td><td>${escapeHtml(item.processTime||'Invalid Date')}</td>`;
        tableBody.appendChild(row);
      });
    }
    updateStats();
  ...
  (function setupFlatpickr(){
    ...
    if (typeof flatpickr === 'undefined') { datePickerDiv.addEventListener('click', e => { e.preventDefault(); alert('Calendar library not loaded.'); }); return; }
    ...
    const fp = flatpickr(dateInput, { mode:'range', dateFormat:'d-m-Y', defaultDate: defaultDates, clickOpens:false, onChange(selectedDates, dateStr, instance){
      if (selectedDates.length === 2) { dateRange.textContent = `${instance.formatDate(selectedDates[0],"d-m-Y")} - ${instance.formatDate(selectedDates[1],"d-m-Y")}`; selectedRange.start = selectedDates[0]; selectedRange.end = selectedDates[1]; }
      else if (selectedDates.length === 1) { dateRange.textContent = instance.formatDate(selectedDates[0],"d-m-Y"); selectedRange.start = selectedDates[0]; selectedRange.end = selectedDates[0]; }
      else { dateRange.textContent = 'All dates'; selectedRange = { start:null, end:null }; }
      applyFiltersAndSort();
    }});
  ...
  async function saveEntryToFirestore(entry){
    try {
      ...
      if (!response.ok) { const txt = await response.text().catch(()=>null); console.error('[Backend] saveEntry returned', response.status, txt); return; }
      const data = await response.json().catch(()=>({success:false}));
      if (!data.success) console.error('[Backend] Save failed:', data); else console.log('[Backend] Entry saved.');
  ...
  async function subscribeToData(){
    try {
      const res = await chromeStorageGet('kosh_auth');
      const auth = res && res.kosh_auth ? res.kosh_auth : (localStorage.getItem('kosh_auth') ? JSON.parse(localStorage.getItem('kosh_auth')) : null);
      if (!auth || !auth.user || !auth.user.id) console.log('No stored user id — attempting fetch without userId.');
